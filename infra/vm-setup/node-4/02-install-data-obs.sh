...
CREATE USER uitstore WITH PASSWORD '${DATABASE_PASSWORD}';
...
NODE4_IP=$(hostname -I 2>/dev/null | awk '{print $1}')
cd /opt
if [ ! -d "/opt/kafka" ]; then
...
sed -i "s|#advertised.listeners=PLAINTEXT://your.host.name:9092|advertised.listeners=PLAINTEXT://${NODE4_IP:-127.0.0.1}:9092|" \
...
ufw allow 3000/tcp
ufw allow from "${VM1_IP}" to any port 9090 proto tcp
ufw allow from "${VM2_IP}" to any port 5432 proto tcp
ufw allow from "${VM3_IP}" to any port 5432 proto tcp
ufw allow from "${VM2_IP}" to any port 9092 proto tcp
ufw allow from "${VM3_IP}" to any port 9092 proto tcp
ufw allow from "${VM2_IP}" to any port 5044 proto tcp
ufw allow from "${VM3_IP}" to any port 5044 proto tcp
