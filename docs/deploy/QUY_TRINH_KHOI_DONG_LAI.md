...
- **Thao tác:** Đăng nhập vào NODE-4 và chạy lệnh khởi động:
  ...
  ```
- **Lưu ý cực kỳ quan trọng:** Sau khi chạy lệnh xong, hãy **chờ khoảng 30 - 45 giây**. Các dịch vụ như Kafka và Elasticsearch rất nặng, chúng cần thời gian để "tỉnh ngủ" hoàn toàn.
...
- **Thao tác:** Đăng nhập vào NODE-3 và chạy lệnh khởi động:
  ...
  sudo bash 03-start-all.sh
  ```
- _Mẹo:_ Đoạn script này đã được lập trình thông minh để tự động dùng file `/root/vault-init.txt` để mở khóa (unseal) Vault cho bạn. Bạn không cần làm thủ công.
