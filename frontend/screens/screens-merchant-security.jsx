...
const MerchantSecuritySection = ({ merchantId, user }) => {
  ...
  const load = () => {
    setLoading(true);
    if (!BASE || !user) { setKeyData(null); setAuditLog([]); setLoading(false); return; }
  ...
  return (
    <div>
      <h2 style={{ margin: '0 0 16px', fontSize: 18, fontWeight: 600 }}>Bảo mật shop</h2>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div className="card" style={{ padding: 18 }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--ink-500)' }}>
              Chưa có endpoint backend trả trạng thái MFA cho merchant.
            </div>
          </div>
          ...
          <div className="card" style={{ padding: 18 }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--ink-500)' }}>
              Chưa có endpoint backend trả chứng chỉ mTLS cho merchant.
            </div>
          </div>
            ...
            {filteredLog.length === 0 ? (
              ...
              <>
                ...
                <div style={{ padding: '8px 16px', fontSize: 10, color: 'var(--ink-400)', fontFamily: 'JetBrains Mono, monospace', borderTop: '1px solid var(--ink-100)' }}>
                  Audit log được ký HMAC-SHA256 · immutable · dùng để detect tampering
                </div>
              </>
            )}
          </div>
