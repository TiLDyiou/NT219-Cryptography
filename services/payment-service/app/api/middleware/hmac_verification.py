...
class HmacVerificationMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        # Exclude endpoints: health check, docs, and public webhooks.
        if (
        ...
        # M-09: tự kiểm timestamp tại tầng HMAC (không phụ thuộc nonce guard). Nếu tắt
        # nonce guard, request đã ký vẫn không replay được vĩnh viễn.
        try:
            ts = int(timestamp)
        except ValueError:
            return JSONResponse(
